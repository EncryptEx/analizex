...
@click.option(
    "--top",
    default=5,
    help="The number of rows to show in each category (5 by default)",
)
@click.option(
    "--output",
    default="console",
    help="Specify the type of output (console by default): console/html",
)
@click.option(
    "--categories",
    default="all",
    help="Sets the categories to show (all by default): all/" + "/".join(modesList),
)
@click.option(
    "--ipgeoloc",
    default=False,
    help="Shows IP Geolocation (uses external API), disabled by default",
)
@click.option(
    "--reqtreshold",
    default=1,
    help='Time between requests to determine that they are enumerators/crawlers. 1" by default',
)
